...
服务端启动前必须准备数据库、导入仓库中的初始化 SQL，并保证运行环境能够读取根目录下的 `服务端配置.ini`。
...
文件名目前由 `ServerConfiguration` 写死，不支持通过 `.env`、命令行参数或另一个本地文件名替换。因此本地 Java 运行的工作目录必须是仓库根目录；Docker 则将该文件挂载到 `/app/服务端配置.ini`。

`服务端配置.ini` 当前是 Git 跟踪文件。修改数据库账号、密码和地址后，提交前务必检查：
...
账号密码实现也保留了多代兼容逻辑。当前自动注册使用无盐 SHA-1；登录流程兼容旧哈希和盐化 SHA-512，但部分兼容成功路径会把数据库密码更新为无盐 SHA-1。再叠加明文登录日志，这套认证实现不适合承载真实或复用密码。面向真实用户前需要单独完成密码存储迁移、兼容策略和日志修复，不能只修改配置开关。
...
这些字段通过简单的 `split(",")` 解析，不会自动去除空格或统一去重。事件管理器最终用脚本名作为 Map 键，重复事件名会覆盖先前条目；其他历史列表则可能继续保留重复值。当前跟踪配置中已经存在重复事件名和重复商城屏蔽 ID，而仓库数据测试只检查重复配置键，不检查列表内部重复项。调整列表时应先规范化目标项，并检查 `脚本/` 中是否存在对应实现。
...
- 遗留 `FileoutputUtil` 写入 `日志/logs/` 下的多个 RTF/TXT 文件。

遗留登录代码当前会在登录成功后把账号、**明文密码**、MAC 和远端地址写入 `日志/logs/ACPW.txt`。这是现有实现的高风险限制，不受 `server.logger.packet` 开关保护。开发和联调只能使用隔离测试账号；日志目录必须限制访问，不得上传、提交或对外分享。面向真实用户部署前应先修复该行为并轮换所有曾经使用过的凭据。
